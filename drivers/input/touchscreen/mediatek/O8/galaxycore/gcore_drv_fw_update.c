...
#define FW_BIN_NAME "gcore_tp_fw.bin"
...
/* gcore_reset();  //reset RESX pin */
    ...
    init_completion(&gcore_fw_update_complete);
    GTP_DEBUG("comple done:%d", gcore_fw_update_complete.done);
    if (!wait_for_completion_interruptible_timeout(&gcore_fw_update_complete, 1 * HZ)) {
...
void gcore_request_firmware_update_work(struct work_struct *work)
{
    ...
    if (request_firmware(&fw, gdev_fwu->gcore_nomalfw_rq_name, &gdev_fwu->bus_device->dev)) {
...
static ssize_t fw_update_flash_show(struct device *dev, 
                struct device_attribute *attr, char *buf)
{
    return scnprintf(buf, PAGE_SIZE, "%s\n", gdev_fwu->gcore_nomalfw_rq_name);
}
...
{    
    ...
    switch (enabled)
    {
        ...
        case 7:
            //gcore_fw_event_notify(FW_HANDSFREE_ON);
            tpd_enable_ps(1);
            break;
        case 8:
            //gcore_fw_event_notify(FW_HANDSFREE_OFF);
            tpd_enable_ps(0);
            break;
