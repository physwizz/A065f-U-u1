...
struct alspshub_ipi_data {
	...
	int		ps_cali;
	atomic_t	als_cali;
/*a06 code for AL7160A-98 by zhawei at 20240422 start*/
#ifdef CONFIG_HQ_PROJECT_O8
	atomic_t	als_dim;
#endif    //CONFIG_HQ_PROJECT_O8
/*a06 code for AL7160A-98 by zhawei at 20240422 end*/
                   ...
                   size_t tCount)
{
    ...
    if (enable == 1) {
        ...
        pr_err("Normal calibration!\n");
    }
    /*a06 code for AL7160A-98 by zhawei at 20240422 start*/
    #ifdef CONFIG_HQ_PROJECT_O8
    else if (enable == 2) {
        sensor_selftest_to_hub(ID_LIGHT);
        pr_err("backlight cali!\n");
    }
    #endif //CONFIG_HQ_PROJECT_O8
    /*a06 code for AL7160A-98 by zhawei at 20240422 end*/
    return tCount;
...
/*a06 code for AL7160A-98 by zhawei at 20240422 start*/
#ifdef CONFIG_HQ_PROJECT_O8
	cfg_data[1] = atomic_read(&obj->als_dim);
...
/*a06 code for AL7160A-98 by zhawei at 20240422 start*/
		...
		spin_lock(&calibration_lock);
		atomic_set(&obj->als_dim, event->data[0]);
		atomic_set(&obj->calibration_done, 1);
		spin_unlock(&calibration_lock);
		err = als_dark_cali_report(event->data);
	}
#endif    //CONFIG_HQ_PROJECT_O8
/*a06 code for AL7160A-98 by zhawei at 20240422 end*/
	return err;
...
static int als_set_cali(uint8_t *data, uint8_t count)
{
    ...
    atomic_set(&obj->als_dim, buf[1]);
    if(atomic_read(&obj->als_cali) == 0){
        atomic_set(&obj->als_cali, 1000);
    }
    lcd_info_judge(data);
    pr_err("als_set_cali:[%d, %d]\n", atomic_read(&obj->als_cali), atomic_read(&obj->als_dim));
    spin_unlock(&calibration_lock);
    return sensor_cfg_to_hub(ID_LIGHT, data, 8);
}
#endif    //CONFIG_HQ_PROJECT_O8
/*a06 code for AL7160A-98 by zhawei at 20240422 end*/
...
/*a06 code for AL7160A-114 by lichang at 20240426 start*/
static int ps_set_cali(uint8_t *data, uint8_t count)
{
	...
	struct alspshub_ipi_data *obj = obj_ipi_data;
			...
			int32_t factory_flag = 1;
		#else
			int32_t factory_flag = 0;
		#endif //HQ_FACTORY_BUILD
		int err = 0;

	err = sensor_set_cmd_to_hub(ID_PROXIMITY, CUST_ACTION_SET_FACTORY, &factory_flag);
	if (err < 0) {
		pr_err("%s sensor_set_cmd_to_hub fail, (ID:%d),(action:%d)\n\n", __func__, ID_PROXIMITY, CUST_ACTION_SET_FACTORY);
	}
	#endif //CONFIG_HQ_PROJECT_O8
...
/*a06 code for AL7160A-114 by lichang at 20240426 end*/
